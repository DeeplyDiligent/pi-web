...
before(() => {
  ...
  process.env.PI_WEB_SESSION_SECRET = "test-session-signing-secret";
});
after(() => {
...
test("logs in with one password and reports the signed session", async () => {
  ...
  const { GET: legacySession } = await jiti.import("../session/route.ts");
  const legacyResponse = legacySession(request("GET", undefined, { Cookie: cookiePair }));
