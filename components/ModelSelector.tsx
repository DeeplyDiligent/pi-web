...
interface ModelSelectorProps {
  ...
  refreshing?: boolean;
  catalog?: { checkedAt?: number; warning?: string };
}
...
export function ModelSelector({
  ...
  // Keep the picker anchored when the virtual keyboard changes the viewport.
  useLayoutEffect(() => {
    if (!open) return;
  ...
  return (
    ...
    >
      ...
      {open && anchorRect && (() => {
        ...
        const spaceAbove = anchorRect.top - viewportTop - 14;
        const spaceBelow = viewportTop + viewportHeight - anchorRect.bottom - 14;
        const openAbove = placement === "up" || spaceAbove > spaceBelow;
        const maxHeight = Math.max(96, Math.min(openAbove ? spaceAbove : spaceBelow, viewportHeight * (isMobile ? 0.72 : 0.6)));
        const verticalPosition: CSSProperties = openAbove
            ...
            {onRefresh && (
              <div style={{ padding: "6px 8px", flexShrink: 0, borderBottom: "1px solid var(--border)" }}>
                <button
                  ...
                  style={{ display: "flex", alignItems: "center", gap: 7, minHeight: 40, padding: "4px 8px", border: 0, borderRadius: 6, background: "var(--bg-hover)", color: "var(--text-muted)", cursor: refreshing ? "wait" : "pointer", fontSize: 12, width: "100%" }}
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true" style={{ flexShrink: 0, animation: refreshing ? "spin 0.8s linear infinite" : undefined }}>
                    <path d="M20 7a9 9 0 1 0 1 8M20 3v5h-5" />
                  </svg>
                  {t(refreshing ? "chat.refreshingCopilot" : "chat.refreshCopilot")}
                </button>
                {catalog?.checkedAt && <div style={{ fontSize: 10, color: "var(--text-dim)", padding: "4px 8px 0" }}>{t("chat.copilotLastChecked", { time: new Date(catalog.checkedAt).toLocaleTimeString() })}</div>}
                {catalog?.warning && <div role="status" style={{ fontSize: 11, color: "var(--text-muted)", padding: "4px 8px", maxHeight: 64, overflowY: "auto", whiteSpace: "normal", overflowWrap: "anywhere" }}>{catalog.warning}</div>}
              </div>
            )}
            {showFilter && (
