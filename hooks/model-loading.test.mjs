...
function setup(fetchImpl) {
  ...
  const context = {
    ...
    modelsRequestRef: { current: 0 },
    fetch: fetchImpl,
    MODELS_RETRY_DELAYS_MS: script(schedule.initializer.getText(source)).runInNewContext(),
    delay: async (ms) => { delays.push(ms); },
  };
  for (const name of ["CopilotCatalog", "ModelError", "ModelNames", "ModelScopeWarnings", "ModelThinkingLevels", "ModelThinkingLevelMaps", "ModelList", "NewSessionDefaultModel", "ThinkingLevel"]) {
...
test("manual Copilot refresh uses POST and preserves catalog metadata", async () => {
  const requests = [];
  const catalog = { checkedAt: 123, warning: "Unknown pricing" };
  const state = setup(async (url, options) => {
    requests.push([url, options.method]);
    return Response.json({ models: {}, modelList: [], copilotCatalog: catalog });
  });
  await state.context.loadModels(undefined, true);
  assert.deepEqual(requests, [["/api/models?cwd=%2Fproject", "POST"]]);
  assert.deepEqual(state.writes.find(([name]) => name === "CopilotCatalog"), ["CopilotCatalog", catalog]);
});

test("late model responses cannot overwrite a newer catalog or error state", async () => {
  for (const fail of [false, true]) {
    const pending = Promise.withResolvers();
    let calls = 0;
    const state = setup(async () => {
      if (++calls === 1) return pending.promise;
      return Response.json({ models: { fresh: "Fresh model" }, modelList: [] });
    });
    const old = state.context.loadModels();
    await state.context.loadModels(undefined, true);
    const writesAfterRefresh = [...state.writes];
    pending.resolve(fail
      ? Response.json({ error: "Stale error" }, { status: 503 })
      : Response.json({ models: { stale: "Stale model" }, modelList: [] }));
    if (fail) await assert.rejects(old, /Stale error/);
    else await old;
    assert.deepEqual(state.writes, writesAfterRefresh);
  }
});
