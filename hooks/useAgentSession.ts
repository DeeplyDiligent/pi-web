...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const loadSession = useCallback(async (
    ...
    try {
      ...
      if (res.status === 404) {
        if (options?.strict) throw new Error("Session not found");
        if (showLoading) {
          setData(null);
  ...
  const handleEdit = useCallback(async (entryId: string | null, message: UserMessage): Promise<UserMessage | null> => {
    ...
    try {
      ...
      setQueuedMessages({ steering: [], followUp: [] });
      settleUiStage();
      await loadSession(sid, false, false, { strict: true });
  ...
  const loadModels = useCallback(async (signal?: AbortSignal, force = false) => {
    ...
    try {
      ...
      if (requestId === modelsRequestRef.current && !signal?.aborted && !(e instanceof DOMException && e.name === "AbortError")) {
  ...
  useEffect(() => {
    ...
    const requestCounter = modelsRequestRef;
    (async () => {
