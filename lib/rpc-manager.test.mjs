...
test("fork_branch snapshots a completed response while the source keeps running", async () => {
  ...
  manager.appendMessage({ role: "user", content: "keep running on the original", timestamp: Date.now() });
  ...
  try {
    ...
    assert.equal(manager.getLeafId(), liveLeafId);
    assert.equal(wrapper.inner.isStreaming, true);
