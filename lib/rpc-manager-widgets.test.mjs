...
test("renders a factory widget immediately with the fixed headless facade", async () => {
  ...
  for (const color of ["scrollbarTrack", "scrollbarThumb", "thinkingMax", "searchMatchText"]) {
    assert.equal(theme.fg(color, "plain text"), "plain text");
    assert.equal(theme.getFgAnsi(color), "");
  }
