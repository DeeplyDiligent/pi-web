...
| `ENV_TYPE=work` | Add a briefcase badge to the mobile app icon | Plain blue π icon |
...
The installed app uses a white π on blue, with an edge-to-edge maskable icon for Android launchers. Set `ENV_TYPE=work` on the **server process** to add a bottom-right briefcase badge, for example `ENV_TYPE=work pi-web`. Other values (or an unset variable) use the plain icon. Restart the server after changing this setting; no rebuild is needed. The manifest and Apple touch-icon endpoint select the same variant at runtime.

Android may take time to refresh an installed app's icon. Close and reopen the app; if it stays stale, reinstall the PWA. Android's **Themed icons** setting can replace app colours with the wallpaper palette; disable that setting to see the supplied blue and white colours.
