...
| `ENV_TYPE=work` | Use the maroon mobile app icon with a briefcase badge | Plain blue π icon |
...
The installed app uses a white π on blue, with an edge-to-edge maskable icon for Android launchers. Set `ENV_TYPE=work` on the **server process** to use the maroon variant with a bottom-right briefcase badge, for example `ENV_TYPE=work pi-web`. Other values (or an unset variable) use the plain blue icon. Restart the server after changing this setting; no rebuild is needed. The manifest and Apple touch-icon endpoint select the same variant at runtime.

Android may take time to refresh an installed app's icon. Close and reopen the app; if it stays stale, reinstall the PWA. Android's **Themed icons** setting can replace app colours with the wallpaper palette; disable that setting to see the supplied colours.

Artwork is generated by `python3 bin/generate-app-icons.py` (requires Pillow). Both the π and work badge stay within Android's maskable safe zone. Use new asset filenames when changing the artwork so launcher and service-worker caches can distinguish revisions.
